...
public class AdminSellOrderCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String @NonNull [] args) {
        ...
        if (args.length < 3) {
            player.sendMessage("§cUsage: /adminsellorder <item> <amount> <price>");
            return true;
        }
        ...
        org.bukkit.block.Sign sign = shopManager.createShopSign(player, chestBlock, "SELL", mat, amount, price, true);
        if (sign == null) return true;

        shopManager.createShop(chestBlock.getLocation(), player.getUniqueId(), "SELL", mat.name(), amount, price, sign.getLocation(), true);
        player.sendMessage("§aSuccessfully created an ADMIN selling shop! (unlimited stock, no payout to owner)");
        return true;
    }
}