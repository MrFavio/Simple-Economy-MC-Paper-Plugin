...
public class BuyOrderCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String @NonNull [] args) {
        ...
        if (args.length < 3) {
            player.sendMessage("§cUsage: /buyorder <item> <amount> <price>");
            return true;
        }
        ...
        org.bukkit.block.Sign sign = shopManager.createShopSign(player, chestBlock, "BUY", mat, amount, price, false);
        if (sign == null) return true;

        shopManager.createShop(chestBlock.getLocation(), player.getUniqueId(), "BUY", mat.name(), amount, price, sign.getLocation(), false);
        player.sendMessage("§aSuccessfully created a buying order shop!");
        return true;
    }
}