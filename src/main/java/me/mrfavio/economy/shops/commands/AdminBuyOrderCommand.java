...
public class AdminBuyOrderCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String @NonNull [] args) {
        ...
        if (args.length < 3) {
            player.sendMessage("§cUsage: /adminbuyorder <item> <amount> <price>");
            return true;
        }
        ...
        org.bukkit.block.Sign sign = shopManager.createShopSign(player, chestBlock, "BUY", mat, amount, price, true);
        if (sign == null) return true;

        shopManager.createShop(chestBlock.getLocation(), player.getUniqueId(), "BUY", mat.name(), amount, price, sign.getLocation(), true);
        player.sendMessage("§aSuccessfully created an ADMIN buying order shop! (unlimited funds, no owner payout)");
        return true;
    }
}